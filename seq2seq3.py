...
def input_prompt_mask(text, tokenizer):
    prompt = (f'対話記録：{text}　{tokenizer.sep_token}'
              'エンティティタイプ：方面、県、サブエリア、位置情報コピー、大ジャンル、中ジャンル、小ジャンル、特徴条件は、キーワード。'
              f'上記の会話記録に記載されているエンティティは{tokenizer.mask_token}です。')
    return prompt
        ...
        if self.mode=='train':
            ...
            if self.config.constraint_decode:
                ...
                key_val_text = tokenizer.decode(
                    model.module.generate(input_ids=batch['input_ids'].to(device), do_sample=False)[0],
    ...
    def construct_instance(self, data, tokenizer, is_train, is_chinese):
        ...
        for record in data:
            if any([kw in self.config.model_name for kw in ['bart']]):
                input_text = input_prompt_mask(record['src'], tokenizer)
